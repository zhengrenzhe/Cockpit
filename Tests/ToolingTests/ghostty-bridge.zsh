...
int main(int argc, char **argv) {
  ...
  @autoreleasepool {
    ...
    cockpit_ghostty_renderer_resize(renderer, UINT32_MAX, UINT32_MAX, 1.0);
    assert(cockpit_ghostty_renderer_apply(renderer, (const uint8_t *)delta.bytes, delta.length) == -1);
    cockpit_ghostty_renderer_resize(renderer, 1280, 960, 2.0);
    assert(cockpit_ghostty_renderer_apply(renderer, (const uint8_t *)delta.bytes, delta.length) == 1);
    assert(cockpit_ghostty_renderer_apply(renderer, (const uint8_t *)delta.bytes, delta.length) == -1);
    ...
    cockpit_ghostty_renderer_resize(renderer, 800, 600, 1.5);
    assert(layer.contentsScale == 1.5);
    assert(layer.drawableSize.width == 800 && layer.drawableSize.height == 600);
    cockpit_ghostty_renderer_set_visible(renderer, false);
    assert(cockpit_ghostty_renderer_apply(renderer, (const uint8_t *)snapshot.bytes, snapshot.length) == 0);
    cockpit_ghostty_renderer_set_visible(renderer, true);
    assert(cockpit_ghostty_renderer_apply(renderer, (const uint8_t *)snapshot.bytes, snapshot.length) == 1);
